...
async def command_start_handler(message: Message) -> None:
    ...
    if start_param:
        ...
        # Отправляем приветственное сообщение с информацией о реферальной программе
        welcome_text = f"""
Привет, {user_name}! 👋

Добро пожаловать! Вы перешли по реферальной ссылке. 
Для продолжения работы с приложением нажмите кнопку ниже:
        """
...
async def handle_message(message: Message) -> None:
    ...
    """
    # Просто отправляем кнопку для открытия WebApp
    await message.answer(
